...
public abstract class ChunkProcessorAllChunks
{
    private boolean areCoordinatesValid(BlockPos pos1, BlockPos pos2, Level world) throws CommandSyntaxException
    {
        ...
        {
            throw (new SimpleCommandExceptionType(Component.translatable("Argument(s) out of range: y < " + minY))).create();
        }

        if (pos1.getY() > maxY || pos2.getY() > maxY)
        {
            throw (new SimpleCommandExceptionType(Component.translatable("Argument(s) out of range: y > " + maxY))).create();
        }

        if (pos1.getX() < -30000000 || pos2.getX() < -30000000 || pos1.getZ() < -30000000 || pos2.getZ() < -30000000)
        {
            throw (new SimpleCommandExceptionType(Component.translatable("Argument(s) out of range (world limits): x or z < -30M"))).create();
        }
    ...
    public void processChunks(Level world, BlockPos posMin, BlockPos posMax) throws CommandSyntaxException
    {
        ...
        {
            throw (new SimpleCommandExceptionType(Component.translatable("Invalid coordinate(s) in the range, aborting"))).create();
        }
